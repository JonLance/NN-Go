...
void NNGo::forwardPropagate(const std::vector<std::vector<std::vector<double>>>& X) {
    ...
    hidden.assign(HIDDEN_LAYERS, std::vector<std::vector<std::vector<double>>>(HIDDEN_CHANNELS, std::vector<std::vector<double>>(BOARD_SIZE, std::vector<double>(BOARD_SIZE, 0.0))));
...
void NNGo::backwardPropagate(const std::vector<double>& y_true, double reward, const std::vector<double>& territoryTarget) {
    ...
    for (int o = 0; o < OUTPUT_SIZE; ++o) {
        double d = (probs[o] - y_true[o]) * reward;
        if (d > 5.0) d = 5.0;
        if (d < -5.0) d = -5.0;
        delta_out[o] = d;
    }
    ...
    std::vector<std::vector<std::vector<std::vector<double>>>> delta_hidden(
        HIDDEN_LAYERS, std::vector<std::vector<std::vector<double>>>(
    ...
    for (int o = 0; o < OUTPUT_SIZE; ++o) {
        ...
        for (int ch = 0; ch < HIDDEN_CHANNELS; ++ch) {
            for (int r = 0; r < BOARD_SIZE; ++r) {
                for (int c = 0; c < BOARD_SIZE; ++c) {
                    weights_out[o][ch][r][c] -= learning_rate * delta_out[o] * hidden[HIDDEN_LAYERS-1][ch][r][c];
                }
            }
        }
    }
...
void NNGo::trainOnEpisodes(const std::vector<memstep>& episodes, CellState winner, double finalMargin) {
    for (const memstep& step : episodes) {
        ...
        if(step.captsMade > 0) {
            ...
            if (step.captsMade > 1) reward += (step.captsMade * step.captsMade) * 0.8;
        }
        if(step.stonesLost > 0) {
            reward -= step.stonesLost * 1.0;
            if (step.stonesLost > 2) reward -= (step.stonesLost * step.stonesLost) * 0.8;
        }
